...
import { createDosen } from "@/services/admin/dosen/create-dosen";
import {
  UserPlus,
  User as UserIcon,
  GraduationCap,
...
export type CreateDosenPayload = z.infer<typeof createDosenSchema>;

export default function CreateDosenModal() {
  const { closeModal } = useDataTableModal();
  const form = useForm<CreateDosenPayload>({
  ...
  const onSubmit = (values: CreateDosenPayload) => {
      ...
      <div className="border-b border-primary/10 bg-primary/5 px-6 pb-6 pt-8">
        <DialogHeader className="flex flex-col items-center justify-center text-center">
          ...
          <DialogDescription className="mt-1 max-w-[320px] font-medium text-slate-500">
            Silakan lengkapi data profil dan bidang keahlian dosen di bawah ini.
          </DialogDescription>
        </DialogHeader>
      </div>
                    ...
                    <FormControl>
                      <Select
                        ...
                        disabled={
                          ...
                          !form.watch("bidang_penelitian_major_id") ||
                          availableMinors.length === 0
                        }
                      >
                        <SelectTrigger className="h-11 border-slate-200 bg-slate-50/50 font-medium italic transition-all focus:bg-white">
                          <SelectValue
                            placeholder={
                              !form.watch("bidang_penelitian_major_id")
                                ? "Pilih bidang utama terlebih dahulu"
                                : "Tambah bidang minor (opsional)"
                            }
                          />
                        </SelectTrigger>
                        <SelectContent>
